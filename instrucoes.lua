...
local group = self.view
 ...
 --Imagem de fundo e posições x e y
    bg = display.newImage( "images/instrucoes/background-menu.jpg" )
    bg.x = display.contentWidth/2
    bg.y = display.contentHeight/2
    group:insert(bg)

    --Som do jogo
    bg = audio.loadStream( "sounds/instrucoes/inst.mp3" )
    audio.play( bg, { channel = 1, loops = -1 } )

    simbol = display.newImage( "images/instrucoes/inst.png" )
    simbol.x = display.contentWidth/2
    simbol.y = display.contentHeight/2 + 10
    
    group:insert(simbol)

    logoquest = display.newImage( "images/instrucoes/logo.png" )
    logoquest.x = display.contentWidth/2
    logoquest.y = display.contentHeight/2 - 190
    group:insert(logoquest)

    menu = display.newImage( "images/instrucoes/menu.png" )
    menu.x = display.contentWidth/2
    menu.y = display.contentHeight/2 + 190
    menu.xScale = 0.8
    menu.yScale = 0.8
    group:insert(menu)
